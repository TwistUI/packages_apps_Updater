...
updates_dir=/data/twistui_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have TwistUI-VERSION-DATE-TYPE-* as format"
...
# Assume TwistUI-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.twistui.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.twistui.updater/databases/updates.db" \
